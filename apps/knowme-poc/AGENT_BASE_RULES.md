...
```
Component → Hook → Zustand Store → Service → API
```

Flutter:

```
Widget → Controller/Notifier → Riverpod Provider → Service → API
```

Rust HTMX:

```
Template → Handler → Store/Domain Layer → Service → Repository
```
...
```
Component → View Model → Store → Service → API
```
