...
const ThreadScrollToBottom: FC = () => {
  return (
    <ThreadPrimitive.ScrollToBottom render={<TooltipIconButton tooltip="Scroll to bottom" variant="secondary" className="aui-thread-scroll-to-bottom absolute -top-12 z-10 self-center rounded-full p-4 shadow-none disabled:invisible" />}><ArrowDownIcon /></ThreadPrimitive.ScrollToBottom>
  );
};
...
const ThreadSuggestionItem: FC = () => {
  return (
    <div className="aui-thread-welcome-suggestion-display fade-in slide-in-from-bottom-2 animate-in fill-mode-both duration-200">
      <SuggestionPrimitive.Trigger send render={<Button variant="secondary" className="aui-thread-welcome-suggestion text-foreground hover:bg-accent h-auto gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-normal whitespace-nowrap shadow-none transition-colors" />}><SuggestionPrimitive.Title className="aui-thread-welcome-suggestion-text-1" /><SuggestionPrimitive.Description className="aui-thread-welcome-suggestion-text-2 empty:hidden" /></SuggestionPrimitive.Trigger>
    </div>
  );
};
...
const Composer: FC = () => {
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      <ComposerPrimitive.AttachmentDropzone render={<div data-slot="aui_composer-shell" className="flex w-full flex-col gap-2 rounded-(--composer-radius) bg-(--composer-bg) p-(--composer-padding) shadow-none transition-colors focus-within:bg-muted data-[dragging=true]:bg-accent" />}><ComposerAttachments /><ComposerPrimitive.Input
...
const MessageError: FC = () => {
  return (
    <MessagePrimitive.Error>
      <ErrorPrimitive.Root className="aui-message-error-root bg-destructive/10 text-destructive dark:bg-destructive/5 mt-2 rounded-md p-3 text-sm dark:text-red-200">
        ...
        >
          {({ part, children }) => {
            switch (part.type) {
              ...
              case "text":
                return (
                  <div className="my-2 w-fit max-w-[88%] rounded-2xl rounded-es-md bg-[color:var(--color-card)] px-4 py-3 text-[color:var(--color-fg)]">
                    <MarkdownText />
                  </div>
                );
              case "reasoning":
    ...
    >
      ...
      <ActionBarMorePrimitive.Root>
        ...
        <ActionBarMorePrimitive.Content
          ...
          align="start"
          sideOffset={6}
          className="aui-action-bar-more-content bg-popover/95 text-popover-foreground data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:animate-in data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=closed]:animate-out data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 z-50 min-w-[8rem] overflow-hidden rounded-xl border-0 p-1.5 shadow-none backdrop-blur-sm"
        >
  ...
  return (
    ...
    >
      ...
      <div className="aui-user-message-content-wrapper relative col-start-2 min-w-0">
        <div className="aui-user-message-content peer max-w-[88%] bg-[color:var(--color-ember-soft)] text-foreground rounded-2xl rounded-ee-md px-4 py-3 wrap-break-word empty:hidden">
...
const EditComposer: FC = () => {
  return (
    ...
    >
      <ComposerPrimitive.Root className="aui-edit-composer-root ms-auto flex w-full max-w-[85%] flex-col rounded-(--composer-radius) border-0 bg-(--composer-bg) shadow-none">
