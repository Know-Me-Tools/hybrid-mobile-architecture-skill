...
### Corrections to the initial research (found at the compiler, 2026-07-16)

Three things the API research got wrong; all caught by building, not by reading:

1. **Errors are two unrelated types.** `GgufModelBuilder::build()` returns
   `anyhow::Result<Model>`, but `Model::stream_chat_request()` returns
   `mistralrs::error::Result<Stream<'_>>`. A single `map_err` over
   `mistralrs::error::Error` doesn't compile. `classify()` takes a rendered
   `String` so it serves both; the builder path uses `{e:#}` to capture anyhow's
   full cause chain, where the real reason (404, corrupt GGUF, OOM) lives.
2. **There is no `with_max_seq_len`.** GgufModelBuilder exposes no
   sequence-length override — a GGUF's context length is baked into the file.
   `LocalModelSpec.context_len` is therefore unused on this lane and stays in the
   trait for llama-cpp-2 (mobile), which does accept one.
3. **`Response::ValidationError`/`InternalError` carry `Box<dyn Error>`, not
   `String`** — only `ModelError` carries a `String` (plus the partial response).

### Lifetime note (`Stream<'a>` → `BoxStream<'static>`)

`mistralrs::Stream<'a>` holds `_server: &'a Model` purely as a tether; the data
arrives over an owned `Receiver<Response>`. The trait returns
`BoxStream<'static, StreamEvent>`, so the borrowed stream can't escape directly.
Resolved without unsafe: a spawned task owns the `Arc<Model>` (keeping it alive),
drives the borrowed stream, and forwards `StreamEvent`s over an mpsc channel whose
receiver becomes the `'static` stream. The task also outlives `generate`, so
generation proceeds even if the caller is slow to poll, and a dropped receiver
stops it.

## Dependency conflicts resolved (both pre-existing traps)

1. **regex.** mistral.rs → `serde-saphyr` needs `regex >=1.0, <1.13`; the lock held
   `1.13.1`, pulled transitively by liter-llm as a loose `^1`. Since nothing
   required ≥1.13, `cargo update -p regex --precise 1.12.4` satisfies both lanes.
   Full-workspace clippy confirms the cloud lane is unaffected.
2. **safetensors / candle.** mistral.rs @ b7746a85 declares `safetensors = "0.7.0"`;
   `GQAdonis/candle#main` has moved to `"0.8.0"`. Semver-incompatible → Cargo keeps
   both → `candle-core::Dtype` stops unifying with `mistralrs-quant`'s (E0308,
   `UnsupportedSafeTensorDtype`). **The fork's own crates do not compile together.**
   Fixed with the fallback its Cargo.toml documents: `[patch]` candle to upstream
   `huggingface/candle` rev `5404348` (verified: declares `safetensors = "0.7.0"`).
   Bonus — this removes the floating candle branch from our graph entirely.

## Cargo.lock is now committed (and why it must stay that way)

`apps/knowme-poc/.gitignore` carried a blanket `*.lock` under "# Rust" that silently
ignored `Cargo.lock`, `pubspec.lock`, and `Podfile.lock`. For an application
workspace those belong in git regardless, but here it is load-bearing:

- `GQAdonis/turboquant-rs#main` is still referenced by **branch**; Cargo.lock is the
  only thing pinning it to a reviewed SHA (`b485a88f…`).
- The `regex = 1.12.4` resolution above lives only in the lock. Without it, CI
  re-resolves to 1.13.1 and the build fails exactly as it first did here.
...
- `cargo clippy --workspace --all-targets -D warnings` — clean (cloud lane unaffected).
- Target-gating holds: `mistralrs` is absent from the `wasm32-unknown-unknown` and
  `aarch64-apple-ios` graphs **even with `--features local-mistral` forced on**, and
  absent from desktop's default-feature graph.
- `npx tsc --noEmit` — clean (web lane).

## Remaining (task 3 + 5)

- `tauri-plugin-gen-ui`: depend on `gen_ui_inference` (`features = ["local-mistral"]`),
  add local-inference commands + their autogenerated permissions.
- `gen_ui_agent`: lane selection. `chat::send` already resolves
  `get_model_pref(surface, lane)` with `LANE_CLOUD = "cloud"` — local slots in as a
  second lane behind that same lookup, so `run_stream`'s A2UI adaptation is reused
  as-is rather than duplicated.
- Desktop store: cloud↔local toggle + tok/s display.
- End-to-end smoke test: real model load + generation on desktop Tauri.
