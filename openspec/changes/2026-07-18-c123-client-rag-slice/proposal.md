# 2026-07-18-c123-client-rag-slice
...
knowme-poc vertical slice: pgvector in PGlite (web) and via pglite-oxide (desktop), 384-dim chat-thread embeddings on write, agent data as PEM entities, and the typed retrieval loop (embed -> vector+BM25 -> context -> agent). Mobile sqlite-vec documented + stubbed.
